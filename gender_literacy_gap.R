...
NATIONWIDE_AVG <- read_excel("data_dump/india/DDW-0000C-09.xlsx",sheet=2) %>%
  filter(urbanity=='Total' & area_name=='INDIA' & age_group=='Total') %>%
  filter(religion == 'All religious communities') %>%
  mutate(female_literacy = literate_females/tot_females
         , male_literacy = literate_males/tot_males
...
gl_map <- gl_df_to_map %>%
  ...
  theme_void()+
  labs(title="India's Gender Gap in Literacy (2011)"
  ...
  theme(legend.position = "inside"
        ...
        , legend.position.inside=c(0.8,0.38)
  ...
  annotate("text",label=sprintf("Nationwide gap: %0.1f%%",NATIONWIDE_AVG)
          , x = 93, y=20.3,
